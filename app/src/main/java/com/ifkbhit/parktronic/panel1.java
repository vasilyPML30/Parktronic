...
public class panel1 extends Panel {
    ...
    panel1(Canvas canvas, Resources res, boolean isReverse) {
        super(canvas, res, isReverse);
        reverse = isReverse;
        if (!reverse) {
        ...
        double need_w = k * canvas.getWidth();
        double need_h = canvas.getHeight() * k;
        ...
        panel.setPos(new Point((canvas.getWidth() - panel.img.getWidth()) / 2.0, canvas.getHeight() * (Config.CAR_Y_OFFSET_K / 2) + ((1 - Config.CAR_Y_OFFSET_K) * canvas.getHeight()) / 2.0 - panel.img.getWidth() / 16.0));
        panel.setScaled(1.3);
